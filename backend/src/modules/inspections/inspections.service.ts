...
import { AssetStatus, BookingStatus, InspectionStatus, PaymentStatus, Prisma } from "@prisma/client";
...
export class InspectionsService {
  ...
  /**
   * Create or reuse an inspection session for one booked asset.
   */
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      if (finalAssetStatus === AssetStatus.laundry) {
        await tx.laundryTicket.create({
          data: {
            ...
            bookingId,
            status: "open",
            note: dto.note ?? `T?o t? inspection ${id}`,
          },
        });
      }
      ...
      if (totalPenalty > 0) {
        ...
        await tx.penalty.create({
          data: {
            bookingId,
            reason: dto.note ?? `Inspection findings`,
            amount: totalPenalty,
            createdBy: staffId,
          },
        });
  ...
  async findAssetsNeedingProcessing() {
    ...
    return ok(assets.map((a) => ({
      ...
      garment: {
        id: a.garment_sizes?.garments?.id ?? "",
        name: a.garment_sizes?.garments?.name ?? "",
