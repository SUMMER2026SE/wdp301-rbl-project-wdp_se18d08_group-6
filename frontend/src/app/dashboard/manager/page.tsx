...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  pending_confirmation: { label: "Ch? xác nh?n",        color: "bg-amber-100 text-amber-700" },
  confirmed:            { label: "Ðã xác nh?n",         color: "bg-blue-100 text-blue-700" },
  awaiting_payment:     { label: "Ch? thanh toán",      color: "bg-yellow-100 text-yellow-700" },
  paid:                 { label: "Ðã thanh toán",       color: "bg-green-100 text-green-700" },
  preparing:            { label: "Ðang chu?n b?",       color: "bg-purple-100 text-purple-700" },
  ready_for_pickup:     { label: "S?n sàng nh?n",       color: "bg-teal-100 text-teal-700" },
  delivering:           { label: "Ðang giao",           color: "bg-indigo-100 text-indigo-700" },
  renting:              { label: "Ðang thuê",           color: "bg-lotus/10 text-lotus" },
  returned:             { label: "Ðã tr?",              color: "bg-stone-100 text-stone-600" },
  ...
  cancelled:            { label: "Ðã h?y",              color: "bg-red-100 text-red-600" },
  rejected:             { label: "T? ch?i",             color: "bg-red-100 text-red-700" },
  overdue:              { label: "Quá h?n",             color: "bg-red-200 text-red-800" },
};

const ASSET_STATUS_META: Record<string, { label: string; color: string }> = {
  available:         { label: "S?n sàng",       color: "bg-state-available/10 text-state-available border border-state-available/20" },
  reserved:          { label: "Ðã gi? ch?",     color: "bg-amber-100 text-amber-700 border border-amber-200" },
  rented:            { label: "Ðang thuê",      color: "bg-state-rented/10 text-state-rented border border-state-rented/20" },
  ...
  maintenance:       { label: "B?o trì",        color: "bg-state-maintenance/10 text-state-maintenance border border-state-maintenance/20" },
  damaged:           { label: "Hu h?ng",        color: "bg-state-damaged/10 text-state-damaged border border-state-damaged/20" },
  retired:           { label: "Ðã thanh lý",    color: "bg-stone-100 text-stone-500 border border-stone-200" },
...
const TAB_META: Record<Tab, { title: string; subtitle: string }> = {
  overview:      { title: "T?ng Quan V?n Hành",        subtitle: "Theo dõi doanh thu, don thuê và tình tr?ng kho theo th?i gian th?c." },
  assets:        { title: "Gán Tài S?n",             subtitle: "Gán tài s?n v?t lý cho các don d?t ch?." },
  inventory:     { title: "Qu?n Lý Kho Trang Ph?c",          subtitle: "Qu?n lý m?u trang ph?c, ?nh catalog và tài s?n v?t lý." },
  "inspection-log": { title: "Nh?t Ký Ki?m Tra",     subtitle: "L?ch s? ki?m tra tình tr?ng trang ph?c sau khi tr?." },
  laundry:       { title: "Gi?t S?y",                subtitle: "Qu?n lý hàng ch? gi?t s?y và di?u ph?i." },
  damaged:       { title: "Hu H?ng & M?t",           subtitle: "Báo cáo tài s?n hu h?ng, m?t và b?o trì." },
  finance:       { title: "Ð?i Soát Tài Chính",                subtitle: "Theo dõi doanh thu, ti?n c?c và phí ph?t phát sinh." },
  refunds:       { title: "Duy?t Hoàn C?c",                subtitle: "Duy?t các yêu c?u hoàn c?c ch? x? lý." },
};
...
export default function ManagerDashboardPage() {
  ...
  // Asset assignment state (gán tài s?n)
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setErrorMsg("Có l?i x?y ra khi t?i d? li?u.");
  ...
  async function handleCreateCategory(name: string) {
    ...
    setErrorMsg(res.message ?? "Không th? t?o danh m?c.");
    return null;
  }
  ...
  async function handleCreateGarment(payload: Parameters<typeof createGarment>[0], imagesToAdd: string[]) {
    ...
    if (res.success && res.data) {
      ...
      setErrorMsg(res.message ?? "Không th? t?o trang ph?c.");
    }
    setSubmitting(false);
  }
  ...
  async function handleUpdateGarment(id: string, payload: Parameters<typeof updateGarment>[1], imagesToAdd: string[]) {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.message ?? "Không th? c?p nh?t trang ph?c.");
    }
    setSubmitting(false);
  }
  ...
  async function handleAddImage(garmentId: string, imageUrl: string) {
    ...
    if (!res.success) {
      setErrorMsg(res.message ?? "Không th? thêm ?nh.");
      return;
    }
    await refreshGarments();
  ...
  async function handleCreateAsset(payload: Parameters<typeof createAsset>[0]) {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.message ?? "Không th? t?o tài s?n.");
    }
  }
  ...
  async function handleUpdateAssetStatus(assetId: string, status: string) {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.message ?? "Không th? c?p nh?t tr?ng thái.");
    }
  }
  ...
  async function handleAssignAsset(bookingId: string, itemId: string, itemKey: string) {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.message ?? "Không th? gán tài s?n.");
    }
  }
  ...
  async function handleApproveRefund(refundId: string) {
    ...
    if (res.success) {
      ...
      setErrorMsg(res.message ?? "Không th? duy?t hoàn c?c.");
    }
  }
  ...
  return (
    <ManagerPortalShell
      ...
      onTabChange={goToTab}
      managerName={hasMounted ? (user?.fullName ?? user?.email?.split("@")[0] ?? "Qu?n lý c?a hàng") : "Qu?n lý c?a hàng"}
    ...
    >
      ...
      {loading ? (
        <div className="py-20 text-center text-stone-400">Ðang t?i d? li?u...</div>
      ) : tab === "overview" ? (
        <OverviewTab
...
// -------------------------------------------------------------------------------
// TAB: Assets Assignment (Gán tài s?n)
// -------------------------------------------------------------------------------
    ...
    <div className="space-y-6">
      {bookingsNeedingAssets.length === 0 ? (
        <div className="py-20 text-center text-stone-400">
          ...
          T?t c? don d?u dã du?c gán tài s?n. Không có gì c?n x? lý.
        </div>
      ) : (
            ...
            <div key={booking.id} className="overflow-hidden rounded-xl border border-sand bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)]">
              ...
              <div className="space-y-3 px-6 py-4">
                <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.16em] text-amber-700">
                  ...
                  {unassignedItems.length} item chua gán tài s?n
                </p>
                  ...
                  return (
                    <div key={itemKey} className="flex items-center gap-4 rounded-lg border border-amber-200 bg-amber-50/50 p-4">
                      ...
                      {!state?.open ? (
                        ...
                        <span className="text-sm text-stone-400">Ðang t?i...</span>
                      ) : state.assets.length === 0 ? (
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-red-600">H?t tài s?n kh? d?ng</span>
                          <button type="button" className="text-sm text-stone-500 underline" onClick={() => onClosePicker(itemKey)}>Ðóng</button>
                        </div>
                      ) : (
                        <div className="flex items-center gap-3">
                          <select className="min-w-[200px] rounded-lg border border-sand bg-white px-3 py-2 text-sm outline-none focus:border-antique" value={state.selected} onChange={(e) => onSelectChange(itemKey, e.target.value)} aria-label="Ch?n tài s?n">
                            {state.assets.map((a) => (
                          ...
                          <button type="button" disabled={actioningId === booking.id} className="rounded-lg bg-jade px-4 py-2 text-sm font-semibold text-white transition hover:bg-forest disabled:opacity-50" onClick={() => onAssign(booking.id, item.id, itemKey)}>
                            {actioningId === booking.id ? "..." : "Xác nh?n gán"}
                          </button>
  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <SnapshotCard label="Doanh thu (dang phát sinh)" value={formatVND(totalRentalRevenue)} hint="T? don completed + dang thuê" icon="payments" tone="lotus" />
        ...
        <SnapshotCard label="Ðon d?t ch? hi?n t?i" value={String(activeBookings.length)} hint="Ðang trong lu?ng v?n hành" icon="calendar_month" tone="jade" />
        <SnapshotCard label="Hi?u su?t l?p d?y" value={garments.length === 0 ? "—" : `${utilizationPct}%`} hint={`${rentedItemCount} dang thuê / ${utilizationDenominator} kh? d?ng`} icon="pie_chart" tone="bronze" progress={garments.length === 0 ? null : utilizationPct} />
      </div>

      <section className="rounded-xl border border-sand bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="font-display text-2xl text-ink">Vi?c c?n x? lý</h2>
            <p className="text-sm text-stone-500">Uu tiên v?n hành trong ngày</p>
          </div>
          <span className="material-symbols-outlined text-lotus">task_alt</span>
        </div>
        <div className="grid gap-3 md:grid-cols-4">
          <QuickWorkItem icon="swap_horiz" label="C?n gán tài s?n" value={bookingsNeedingAssets.length} tone="amber" onClick={() => onGoToTab("assets")} />
          <QuickWorkItem icon="search_check" label="Ch? ki?m tra" value={returnedWaiting} tone="orange" onClick={() => onGoToTab("inspection-log")} />
          <QuickWorkItem icon="dry_cleaning" label="Ðang gi?t s?y" value={laundryTickets.length} tone="blue" onClick={() => onGoToTab("laundry")} />
          <QuickWorkItem icon="build" label="C?n b?o trì" value={maintenanceJobs.filter((j) => j.status !== "completed" && j.status !== "cannot_repair").length} tone="red" onClick={() => onGoToTab("damaged")} />
        </div>
      </section>
      ...
      <section className="rounded-xl border border-sand bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="font-display text-2xl text-ink">T?ng Quan Kho Tài S?n</h2>
            <p className="text-sm text-stone-500">Tình tr?ng hi?n v?t dang v?n hành trong c?a hàng</p>
          </div>
          <span className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-400">{effectiveAssets.length} tài s?n</span>
        </div>
      ...
      <div className="grid grid-cols-1 gap-gutter lg:grid-cols-3">
        <div className="rounded-xl border border-sand bg-white p-6 shadow-[0_4px_12px_rgba(74,4,4,0.03)] lg:col-span-2">
          <h2 className="mb-6 font-display text-2xl text-ink">Tình Tr?ng V?n Hành</h2>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
            <StatusCell label="Ch? x? lý" value={countBy(["pending_confirmation", "awaiting_payment"])} tone="amber" />
            <StatusCell label="Ðang chu?n b?" value={countBy(["paid", "preparing", "ready_for_pickup", "delivering"])} tone="purple" />
            <StatusCell label="Ðang thuê" value={countBy(["renting"])} tone="lotus" />
          ...
          <div className="mt-6 flex items-start gap-4 rounded-r-lg border-l-4 border-lotus bg-[#fff4ef] p-4">
            ...
            <div>
              <h3 className="text-sm font-semibold text-ink">C?nh Báo V?n Hành</h3>
              <p className="mt-1 text-sm text-stone-600">
                {bookingsNeedingAssets.length > 0
                  ? `${bookingsNeedingAssets.length} don dang ch? gán tài s?n. C?n x? lý ngay d? không tr? ti?n d? giao trang ph?c.`
                  : "Không có c?nh báo. T?t c? don dang ch? d?u dã du?c gán tài s?n."}
              </p>
              {bookingsNeedingAssets.length > 0 && (
                <button type="button" onClick={() => onGoToTab("assets")} className="mt-2 text-sm font-semibold text-lotus underline hover:text-oxblood">
                  Ði t?i kho trang ph?c ?
                </button>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col rounded-xl border border-sand bg-white p-6 shadow-[0_4px_12px_rgba(74,4,4,0.03)]">
          ...
          <div className="flex flex-1 flex-col gap-3">
            ...
            <ShortcutButton icon="fact_check" label="Nh?t ký ki?m tra" tone="jade" onClick={() => onGoToTab("inspection-log")} />
    ...
    <div className="flex h-[calc(100vh-240px)] -mx-4 sm:-mx-6 lg:-mx-8 overflow-hidden border-y border-sand bg-white">
      ...
      <section className="w-1/2 flex flex-col border-r border-sand bg-warm-ivory">
        <div className="p-4 border-b border-sand bg-surface-container-low sticky top-0 z-10 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <h2 className="font-display text-xl text-ink">Danh m?c Tuy?t tác</h2>
        ...
        <div className="border-b border-sand bg-white px-4 py-3">
          <div className="relative">
            ...
            <input value={garmentSearch} onChange={(e) => setGarmentSearch(e.target.value)} className="w-full rounded-lg border border-sand bg-[#fff8f6] py-2 pl-10 pr-3 text-sm outline-none focus:border-antique" placeholder="Tìm trang ph?c, danh m?c, size..." />
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-4">
          {filteredGarments.length === 0 ? (
            <div className="py-20 text-center text-stone-400">Chua có trang ph?c.</div>
          ) : (
      ...
      <section className="w-1/2 flex flex-col bg-surface">
        <div className="p-4 border-b border-sand bg-surface-container-low sticky top-0 z-10 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <h2 className="font-display text-xl text-ink">Qu?n lý Hi?n v?t</h2>
            <span className="text-xs text-stone-500">{assets.length} tài s?n</span>
          </div>
          {selectedGarmentId && (
        ...
        <div className="border-b border-sand bg-white px-4 py-3">
          <div className="flex gap-2">
            <div className="relative flex-1">
              <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-[18px] text-stone-400">search</span>
              <input value={assetSearch} onChange={(e) => setAssetSearch(e.target.value)} className="w-full rounded-lg border border-sand bg-[#fff8f6] py-2 pl-10 pr-3 text-sm outline-none focus:border-antique" placeholder="Tìm mã tài s?n..." />
            </div>
            <select value={assetStatusFilter} onChange={(e) => setAssetStatusFilter(e.target.value)} className="rounded-lg border border-sand bg-white px-3 py-2 text-sm outline-none focus:border-antique" aria-label="L?c tr?ng thái tài s?n">
        ...
        {assetsLoading ? (
          <div className="flex-1 flex items-center justify-center text-sm text-stone-400">Ðang t?i...</div>
        ) : filteredAssets.length === 0 ? (
          <div className="flex-1 flex items-center justify-center text-sm text-stone-400">
            {selectedGarmentId ? "Chua có tài s?n cho m?u này." : "Ch?n m?t m?u trang ph?c."}
          </div>
        ) : (
            ...
            <div className="flex-1 overflow-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container-highest border-b border-outline-variant sticky top-0">
                  <tr>
                    <th className="font-label-sm text-label-sm text-on-surface-variant py-3 px-4 font-semibold uppercase tracking-wider">Mã hi?n v?t</th>
                    <th className="font-label-sm text-label-sm text-on-surface-variant py-3 px-4 font-semibold uppercase tracking-wider">Tr?ng thái</th>
                    <th className="font-label-sm text-label-sm text-on-surface-variant py-3 px-4 font-semibold uppercase tracking-wider">Tình tr?ng</th>
                  </tr>
                </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-4 text-on-surface-variant">
                          {a.conditionNote ?? (a.status === "available" ? "T?t" : "—")}
                        </td>
                      </tr>
                    );
                  })}
            ...
            {selectedAsset && (
              <aside className="w-80 bg-surface-container-low border-l border-outline-variant flex flex-col flex-shrink-0 shadow-[-4px_0_15px_-3px_rgba(74,4,4,0.05)] z-20">
                ...
                <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
                  ...
                  <div>
                    <label className="font-label-sm text-label-sm text-on-surface-variant uppercase mb-1 block">C?p nh?t tr?ng thái</label>
                    <select
                  ...
                  <div>
                    ...
                    {assetHistoryLoading ? (
                      <p className="text-xs text-stone-400">Ðang t?i...</p>
                    ) : assetHistory.length === 0 ? (
                      <p className="text-xs text-stone-400">Chua có l?ch ki?m tra.</p>
                    ) : (
                        ...
                        {assetHistory.slice(0, 5).map((h) => (
                          <div key={h.id} className="relative">
                            ...
                            <div className="font-label-sm text-label-sm text-on-surface-variant mb-1">
                              {h.createdAt.slice(0, 10)} — {h.status === "completed" ? "Ðã ki?m tra" : h.status}
                            </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl border border-sand bg-white p-6 shadow-2xl">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="font-display text-2xl text-ink">{garment ? "S?a trang ph?c" : "Thêm trang ph?c m?i"}</h3>
        ...
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Tên trang ph?c *</label>
            <input value={name} onChange={(e) => setName(e.target.value)} required className="w-full rounded-lg border border-sand px-3 py-2 text-sm outline-none focus:border-antique" placeholder="Vd: Nh?t Bình Hoàng Phái" />
          </div>
          <div>
            ...
            <p className="mt-1 text-xs text-stone-400">Danh m?c dùng d? phân lo?i trang ph?c.</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input value={color} onChange={(e) => setColor(e.target.value)} className="w-full rounded-lg border border-sand px-3 py-2 text-sm outline-none focus:border-antique" placeholder="Vd: Ð?" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Giá thuê/ngày (VNÐ) *</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Ti?n c?c (VNÐ) *</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Mô t?</label>
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} className="w-full rounded-lg border border-sand px-3 py-2 text-sm outline-none focus:border-antique" placeholder="Mô t? trang ph?c..." />
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-sand bg-[#fff8f6] p-4">
          <div className="flex items-center justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-xs text-stone-500">T?i ?nh lên ho?c dùng URL d? hi?n th?.</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded border border-sand bg-white p-3">
              <label className="mb-2 block text-xs font-semibold uppercase tracking-wider text-stone-500">T?i ?nh lên (File)</label>
              ...
              {uploadingImage && <p className="mt-1 text-[10px] text-stone-400">Ðang t?i lên...</p>}
            </div>
            <div className="rounded border border-sand bg-white p-3">
              <label className="mb-2 block text-xs font-semibold uppercase tracking-wider text-stone-500">Ho?c dùng URL ?nh</label>
              <div className="flex gap-2">
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button type="submit" disabled={submitting} className="rounded-lg bg-lotus px-6 py-2.5 text-sm font-semibold text-white hover:bg-oxblood disabled:opacity-50">
            {submitting ? "Ðang luu..." : garment ? "Luu thay d?i" : "T?o trang ph?c"}
          </button>
        </div>
      </form>
    </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md rounded-2xl border border-sand bg-white p-6 shadow-2xl">
        <div className="mb-6 flex items-center justify-between">
          <h3 className="font-display text-2xl text-ink">Thêm tài s?n m?i</h3>
          <button type="button" onClick={onClose} className="text-stone-500 hover:text-lotus">
            <span className="material-symbols-outlined">close</span>
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Mã tài s?n *</label>
            ...
            <p className="mt-1 text-xs text-stone-400">Mã duy nh?t cho món d? v?t lý.</p>
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Ghi chú tình tr?ng</label>
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold uppercase tracking-wider text-stone-500">Giá mua (VNÐ)</label>
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <button type="submit" disabled={submitting} className="rounded-lg bg-jade px-6 py-2.5 text-sm font-semibold text-white hover:bg-forest disabled:opacity-50">
            {submitting ? "Ðang t?o..." : "T?o tài s?n"}
          </button>
        </div>
      </form>
    </div>
...
function InspectionLogTab({ log, loading }: { log: InspectionLogEntry[]; loading: boolean }) {
  return (
    <div className="space-y-4">
      {loading ? (
        <div className="py-20 text-center text-stone-400">Ðang t?i...</div>
      ) : log.length === 0 ? (
        <div className="py-20 text-center text-stone-400">Chua có phiên ki?m tra nào.</div>
      ) : (
                ...
                <tr key={entry.id} className="transition hover:bg-[#fff8f6]">
                  ...
                  <td className="px-6 py-4">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                      ...
                      {entry.status === "completed" ? "Hoàn t?t" : entry.status}
                    </span>
                  </td>
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        ...
        <div className="py-20 text-center text-stone-400">Không có d? c?n gi?t s?y.</div>
      ) : (
            ...
            <div key={t.id} className="rounded-xl border border-sand bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between">
                ...
                <span className="rounded-full bg-state-laundry/10 text-state-laundry px-2 py-0.5 text-xs font-semibold">
                  {t.status === "open" ? "Ch? gi?t" : "Ðang gi?t"}
                </span>
              </div>
              ...
              <div className="mt-4 flex justify-end">
                ...
                >
                  {actioningId === t.id ? "..." : "Hoàn t?t gi?t"}
                </button>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="space-y-4">
      {loading ? (
        ...
        <div className="py-20 text-center text-stone-400">Không có tài s?n hu h?ng ho?c b?o trì.</div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {jobs.map((j) => (
            <div key={j.id} className="rounded-xl border border-sand bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between">
                ...
                <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                  ...
                  {j.status === "open" ? "C?n x? lý" : j.status === "in_progress" ? "Ðang s?a" : "Hoàn t?t"}
                </span>
              </div>
...
function RefundsTab({
  ...
  return (
    <div className="space-y-6">
      {loading ? (
        <div className="py-20 text-center text-stone-400">Ðang t?i danh sách hoàn c?c...</div>
      ) : refunds.length === 0 ? (
        <div className="py-20 text-center text-stone-400">
          <span className="material-symbols-outlined mb-4 block text-5xl text-stone-200">check_circle</span>
          Không có yêu c?u hoàn c?c nào dang ch? duy?t.
        </div>
      ) : (
        refunds.map((refund) => (
          <div key={refund.id} className="overflow-hidden rounded-xl border border-sand bg-white shadow-[0_4px_20px_rgba(0,0,0,0.04)]">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-sand bg-[#fff8f6] px-6 py-3">
              ...
              <span className="text-xs text-stone-400">
                Yêu c?u lúc {new Date(refund.createdAt).toLocaleString("vi-VN")}
              </span>
            </div>
            <div className="grid gap-6 p-6 sm:grid-cols-2">
              ...
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-500">S? ti?n hoàn</p>
            ...
            <div className="border-t border-sand bg-[#fff8f6] px-6 py-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">Thông tin chuy?n kho?n</p>
              <div className="grid gap-3 text-sm sm:grid-cols-3">
                <div><span className="text-stone-500">Ngân hàng: </span><span className="font-medium text-ink">{refund.bankName ?? "—"}</span></div>
                <div><span className="text-stone-500">S? TK: </span><span className="font-medium text-ink">{refund.bankAccountNumber ?? "—"}</span></div>
                <div><span className="text-stone-500">Ch? TK: </span><span className="font-medium text-ink">{refund.bankAccountHolder ?? "—"}</span></div>
              </div>
            </div>
            <div className="space-y-4 border-t border-sand px-6 py-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Dán URL ?nh ch?p giao d?ch chuy?n kho?n..."
                  value={proofImageUrl}
                  onChange={(e) => onProofImageUrlChange(e.target.value)}
                />
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold text-stone-500">Ghi chú (tu? ch?n)</label>
                <input
                  className="w-full rounded-lg border border-sand px-3 py-2 text-sm outline-none focus:border-antique"
                  placeholder="Ghi chú n?i b?..."
                  value={approveNote}
              ...
              <div className="flex justify-end">
                ...
                >
                  {actioningId === refund.id ? "Ðang x? lý..." : `Duy?t hoàn c?c ${formatVND(refund.amount)}`}
                </button>
              </div>
            </div>
          </div>
    ...
    <div className="space-y-6">
      <div className="grid gap-6 md:grid-cols-3">
        <div className="rounded-xl border border-sand bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-stone-500">T? các don completed + dang thuê</p>
        </div>
        <div className="rounded-xl border border-sand bg-white p-6 shadow-sm">
        ...
        <div className="rounded-xl border border-sand bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-stone-500">Ti?n ph?t phát sinh</p>
          <p className="mt-2 font-display text-3xl text-red-700">{formatVND(totalPenalties)}</p>
          <p className="mt-1 text-sm text-stone-500">T? các l?n ki?m tra phát hi?n hu h?ng</p>
        </div>
      </div>

      <div className="overflow-hidden rounded-xl border border-sand bg-white shadow-sm">
        <div className="border-b border-sand px-6 py-4">
          <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-stone-500">Ð?i soát don g?n dây</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-[#fff8f6] text-xs uppercase tracking-[0.14em] text-stone-500">
              <tr>
                <th className="px-6 py-3">Mã don</th>
                <th className="px-6 py-3">Khách hàng</th>
                <th className="px-6 py-3">Ti?n thuê</th>
            ...
            <tbody className="divide-y divide-sand">
              {bookings.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-stone-400">Chua có don nào.</td>
                </tr>
              ) : (
