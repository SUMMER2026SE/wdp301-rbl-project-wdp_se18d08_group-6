...
export default function StaffProfilePage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage("Cập nhật hồ sơ nhân viên thành công.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="min-h-screen bg-mist text-ink">
      <header className="border-b border-sand bg-mist px-6 py-4">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <div>
            <h1 className="font-display text-3xl text-lotus">Hồ sơ nhân viên</h1>
            <p className="text-sm text-stone-500">Cập nhật thông tin tài khoản nhân viên.</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/dashboard/staff" className="rounded-lg border border-sand bg-white px-4 py-2 text-sm font-semibold text-stone-600 transition hover:border-lotus hover:text-lotus">
              Quay lại dashboard
            </Link>
          ...
          <section className="rounded-xl border border-sand bg-white p-6 shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <p className="truncate text-lg font-semibold text-ink">{user.fullName ?? "Nhân viên cửa hàng"}</p>
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="mt-6 space-y-4">
              <div>
                ...
                <input value={fullName} onChange={(e) => setFullName(e.target.value)} className="w-full rounded-lg border border-sand px-3 py-2 text-sm outline-none focus:border-antique" placeholder="Tên nhân viên" />
              </div>
              <div>
